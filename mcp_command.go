...
func (command *MCPCommand) handleToolsList(request MCPRequest) {
	...
	tools = append(tools, Tool{
		...
		Description: "Get content of specified file in a build. When investigating build problems, AI assistant may use this tool to examine content of some files. Specifically, file \".onedev-buildspec.yml\" defines the job used in the build",
		InputSchema: InputSchema{
			Type: "object",
			Properties: map[string]interface{}{
				"buildReference": map[string]interface{}{
...
func (command *MCPCommand) getCurrentProject() (string, error) {
	if command.currentProjectCache == "" {
		var err error
		_, command.currentProjectCache, err = inferProject(command.workingDir, command.logger)
		if err != nil {
			return "", err
		}
	}
...
func (command *MCPCommand) handlePromptsList(request MCPRequest) {
	...
	prompts := []Prompt{
		...
		},
		{
			...
			Description: "Investigate problems of a build",
			Arguments: []PromptArgument{
				{
					Name:        "buildReference",
					Description: "Reference of the build to investigate problems, for instance #123, project#123, or projectkey-123",
					Required:    true,
				},
				{
					Name:        "instruction",
					Description: "Instruction to investigate problems of the build",
					Required:    false,
				},
			},
		},
...
8. After creating or editing the build spec, call the checkBuildSpec tool again to make sure the new build spec is valid`,
	...
	case "investigate-build-problems":
		reference, err := command.getRequiredStringParam(params, "buildReference")
		if err != nil {
			command.sendError(request.ID, ErrorCodeInvalidParams, "failed to extract buildReference: "+err.Error())
			return
		...
		if instruction != "" {
			promptPrefix = "Investigate problems of build " + reference + ", " + instruction
		} else {
			promptPrefix = "Investigate problems of build " + reference
		}
		messages = []PromptMessage{
			{
				Role: "user",
...
Please follow below steps to investigate problems of the build:
